...
export default class Create extends Command<CreateJSON> {
  ...
  /**
   * Checks if role or account exists on an organisation. If it does,
  ...
  public async run(): Promise<CreateJSON> {
    ...
    if (flags.owner || flags.editor || flags.analyst || flags.viewer) {
      const {results: roles} = await this.api.get<T.ListResponse<T.Role>>(
        `organizations/${this.userConfig.organisation.current}/roles/`
      );
      const {results: accounts} = await this.api.get<T.ListResponse<T.OrgAccount>>(
        `organizations/${this.userConfig.organisation.current}/accounts/`
      );
